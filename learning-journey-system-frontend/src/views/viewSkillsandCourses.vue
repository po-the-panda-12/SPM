<template>
            ...
            <div class="col-md-4" v-for="skill in skillGroup">
              <div v-if="skill.skill_status == 'Active' ">
                <div class="card">
                  ...
                  <button v-if="skill.skill_id === selectedSkillId" class="btn btn-primary disabled">View Courses</button>
                  <button v-else class="btn btn-primary" @click="getCourses(skill.skill_id)">View Courses</button>
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="container" v-if="viewSelectedCourses">
    ...
    <div class="row">
      <AddedCourseCard v-for="course in updatedAvailableCoursesAfterRemoving" :course="course"
        @removeSelectedCourse="removeSelectedCourse(course)"></AddedCourseCard>
    </div>
    <div class="row">
  ...
  <div class="container" v-if="courses">
    ...
    <div class="row">
      <CourseCard v-for="course in updatedAvailableCoursesAfterAdding" :course="course"
        @addCourse="addSelectedCourse(course)" />
    </div>
  </div>
